...
const db = new URI('mysql://root@localhost/leviticus') as DatabaseURI;
...
(async () => {
    const f = db.query(async () => {
        ...
        return await db.query(async () => {
            ...
            const r2 = await db.query(`select 1 as one, 'text' as two, '{}', now(), "key" as nyckel, value as värde from kv as foo where "key" < {limit} order by ${'"key"'} desc`, { limit: 1000 });
